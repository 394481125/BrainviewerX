...
export default function Sidebar() {
  ...
  const [activeTab, setActiveTab] = useState<"layers" | "render">("layers");
  const [plotData, setPlotData] = useState<number[]>([]);
  const canvasRef = useRef<HTMLCanvasElement>(null);

  const showToast = (message: string) =>
    setToastMsg({ text: message, type: "info" });
  ...
  const updateOpacity = (layerId: string, val: number, isVolume: boolean) => {
    if (!nv) return;
    if (isVolume) {
      const index = nv.volumes.findIndex(
        (v: any) => v.id === layerId || v.name === layerId,
      );
      if (index !== -1) {
        nv.setOpacity(index, val);
      }
    } else {
      const mesh = nv.meshes.find(
        (m: any) => m.id === layerId || m.name === layerId,
      );
      if (mesh) {
        mesh.opacity = val;
        nv.updateGLVolume();
  ...
  return (
    <aside className="w-80 bg-gray-950 border-l border-gray-800 flex flex-col shrink-0 text-gray-300">
          ...
          <>
            <div className="flex flex-col space-y-2 mb-4 bg-gray-900/50 p-3 border border-gray-800 rounded">
              ...
              >
                ...
                <option value="https://niivue.github.io/niivue-demo-images/mni152.nii.gz">
                  MNI152 T1 Template
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/spmMotor.nii.gz">
                  SPM Motor Task (spmMotor)
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/BigBrain/bigbrain.nii.gz">
                  BigBrain
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/Allen/AllenAtlas.nii.gz">
                  Allen Atlas
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/CIT168/CIT168toMNI152-2009c_T1w_brain.nii.gz">
                  CIT168 T1w
                </option>
              </select>
              <div className="flex space-x-2 w-full pt-1">
                <button
                  onClick={async () => {
                    ...
                    try {
                      let cmap = "gray";
                      if (sel.value.toLowerCase().includes("motor"))
                        cmap = "warm"; // spmMotor
                      await nv.loadVolumes([
                        { url: sel.value, colormap: cmap, opacity: 1.0 },
                      ]);
                      // In some Niivue versions, loadVolumes triggers onImageLoaded, but let's be safe:
                      if (nv.volumes.length && nv.onImageLoaded)
                ...
                <button
                  onClick={async () => {
                    ...
                    if (nv.volumes.length === 0) {
                      alert(
                        "请先加载一个底图 (Please load a base image first)",
                      );
                      return;
                    }
                    try {
                      let cmap = "warm";
                      if (
                        sel.value.includes("mni152") ||
                        sel.value.includes("bigbrain") ||
                        sel.value.includes("Allen") ||
                        sel.value.includes("CIT168")
                      )
                        cmap = "gray";
            ...
            <div className="flex flex-col space-y-2 mb-4 bg-gray-900/50 p-3 border border-indigo-900/40 rounded">
              <label className="text-[10px] uppercase font-bold tracking-wider text-indigo-300 block pb-1">
                脑区掩膜 (Masks & ROIs)
              </label>
              <select
                id="roi-select"
              ...
              >
                ...
                <option value="https://raw.githubusercontent.com/rordenlab/niivue/main/demos/images/aal.nii.gz">
                  AAL (116区域) - Automated Anatomical Labeling
                </option>
                <option value="https://raw.githubusercontent.com/niivue/niivue/main/packages/niivue/demos/images/aparc.a2009s+aseg.mgz">
                  DK68 (Desikan-Killiany + Destrieux)
                </option>
                <option value="https://raw.githubusercontent.com/niivue/niivue/main/packages/niivue/demos/images/mni152_pveseg.nii.gz">
                  SPM Tissue Maps (PVE Seg)
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/Juelich31/JulichBrainAtlas31_LH.nii.gz">
                  Julich Brain Atlas (HO-like LH)
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/Juelich31/JulichBrainAtlas31_RH.nii.gz">
                  Julich Brain Atlas (HO-like RH)
                </option>
                <option value="https://niivue.github.io/niivue-demo-images/Thalamus/Thalamus_Nuclei-HCP-4DSPAMs_paqd.nii.gz">
                  Thalamus Nuclei 分区
                </option>
              </select>
              <div className="flex space-x-2 w-full pt-1">
                <button
                  onClick={async () => {
                    ...
                    if (nv.volumes.length === 0) {
                      alert(
                        "请先加载一个底图模板 (Please load a base template first)",
                      );
                      return;
                    }
                    try {
                      let cmap = "roi";
                      if (
                        sel.value.includes("aparc") ||
                        sel.value.includes("fs_seg")
                      )
                        cmap = "freesurfer";
                      ...
                      alert(
                        "无法加载该远程掩膜，可能跨域限制或链接失效。请使用下方的「导入自定义 Mask」。",
                      );
                    }
                  }}
            ...
            {layers.length === 0 ? (
              ...
              layers.map((layer) => {
                const actualLayer = layer.isVolume
                  ? nv?.volumes.find(
                      (v: any) => v.id === layer.id || v.name === layer.name,
                    )
                  : nv?.meshes.find(
                      (m: any) => m.id === layer.id || m.name === layer.name,
                    );

                const opacity =
                  layer.isVolume && actualLayer ? actualLayer.opacity : 1.0;
                const isVisible = layer.isVolume
                  ? opacity > 0
                  : actualLayer
                    ? actualLayer.visible
                    : false;
                const cm =
                  (actualLayer && actualLayer.colormap) ||
                  layer.colormap ||
                  "gray";
                ...
                return (
                  <div
                    key={layer.id}
                    className="bg-gray-900 rounded border border-gray-800 text-xs text-gray-400"
                  >
                    <div className="px-3 py-2 border-b border-gray-800/50 flex items-center justify-between group">
                      <div className="flex items-center truncate mr-2">
                        <span className="truncate font-mono" title={layer.name}>
                          {layer.name}
                        </span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <button
                          onClick={() => setInfoLayerId(layer.id)}
                          className="text-gray-500 hover:text-sky-400 transition-colors opacity-0 group-hover:opacity-100"
                          title="详细信息"
                        >
                          <Info size={14} />
                        </button>
                        ...
                        />
                        <button
                          onClick={() => removeLayer(layer.id, layer.isVolume)}
          ...
          <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      type="number"
                      step="0.001"
                      value={nv.scene.crosshairPos[0].toFixed(3)}
                      onChange={(e) =>
                        jumpToCoordinate(0, parseFloat(e.target.value) || 0)
                      }
                      className="w-24 bg-black border border-gray-800 rounded px-2 py-1 text-right text-gray-300 focus:outline-none focus:border-sky-500"
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      type="number"
                      step="0.001"
                      value={nv.scene.crosshairPos[1].toFixed(3)}
                      onChange={(e) =>
                        jumpToCoordinate(1, parseFloat(e.target.value) || 0)
                      }
                      className="w-24 bg-black border border-gray-800 rounded px-2 py-1 text-right text-gray-300 focus:outline-none focus:border-sky-500"
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <input
                      type="number"
                      step="0.001"
                      value={nv.scene.crosshairPos[2].toFixed(3)}
                      onChange={(e) =>
                        jumpToCoordinate(2, parseFloat(e.target.value) || 0)
                      }
                      className="w-24 bg-black border border-gray-800 rounded px-2 py-1 text-right text-gray-300 focus:outline-none focus:border-sky-500"
                    />
                  </div>
            ...
            <div className="bg-gray-900 border border-gray-800 rounded p-3 text-xs">
              ...
              <button
                ...
                className="w-full py-1.5 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded text-[10px] mb-2 transition-colors"
              >
                重置为标准多平面视图
              </button>
            </div>
      ...
      {infoLayerId && (
        <LayerInfoModal
          layer={
            nv?.volumes.find((v: any) => v.id === infoLayerId) ||
            nv?.meshes.find((m: any) => m.id === infoLayerId)
          }
